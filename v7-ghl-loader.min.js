//https://cdn.jsdelivr.net/gh/core-gs/masterstyle-gs@latest/v7-ghl-loader.min.js
//https://cdn.jsdelivr.net/gh/ghlstyle/d-theme@latest/user-d-custom-theme.css

// GHL Loader (unified, strict+fallback, robust DOM watch)
...
// - Adds `loc-<id>` to <html> AND to any element that already has (or later receives) the raw id as a class
...
(function () {
  ...
  // Add .loc-<id> to <html> and to any element that already has the raw id class
    ...
    try {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (node.classList && !node.classList.contains(cls)) {
          node.classList.add(cls); // inject loc-<id>
        }
      }
    } catch (_) {
      // no-op
  ...
  // Watch for late-mounted containers and ensure they receive loc-<id>
  function watchAndEnsureScope(id) {
    if (!id || id === "blank") return;
    var cls = scopeClass(id);
    var start = Date.now();

    // Do an immediate pass
    attachScopeClasses(id);

    // Observe for up to 15s (SPA mounts, route transitions, lazy renders)
    var mo = new MutationObserver(function (muts) {
      for (var k = 0; k < muts.length; k++) {
        ...
        if (m.type === "childList") {
          for (var j = 0; j < m.addedNodes.length; j++) {
            ...
            if (n && n.nodeType === 1) {
              ...
              // Descendants that already have the raw id
        ...
        // Class attribute changes on existing nodes
        if (m.type === "attributes" && m.attributeName === "class") {
          var el = m.target;
          if (el && el.classList && el.classList.contains(id) && !el.classList.contains(cls)) {
            el.classList.add(cls);
          }
        }
      }
  ...
  function applyWithKey(key) {
    ...
    // Scope classes now + keep ensuring for late mounts
  ...
  function start() {
    var cs = currentScript();
    var scopeMode = (cs && cs.getAttribute("data-scope")) || "auto"; // always "auto"
